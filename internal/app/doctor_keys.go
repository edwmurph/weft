...
func keyDoctorAdvice(samples []keyDoctorSample) string {
	...
	if hasOptionBackspace && strings.HasPrefix(optionBackspace.Label, "alt+") {
		return "OK: Option+Backspace is distinguishable. Weft can handle it and forward it into the Console pane.\n"
	}
...
func updateIterm2OptionBackspaceMapping(data map[string]any, profileName string) (string, bool, error) {
	bookmarks, ok := data["New Bookmarks"].([]any)
	if !ok || len(bookmarks) == 0 {
		return "", false, errors.New("iTerm2 preferences do not contain profiles")
	}
	profile, resolvedName, err := selectIterm2Profile(data, bookmarks, profileName)
	if err != nil {
		return "", false, err
	}
	keyboardMap, _ := profile["Keyboard Map"].(map[string]any)
	if keyboardMap == nil {
		keyboardMap = map[string]any{}
		profile["Keyboard Map"] = keyboardMap
	}
	if entry, ok := keyboardMap[iTerm2OptionBackspaceKey].(map[string]any); ok && iterm2KeyboardEntryMatches(entry, iTerm2OptionBackspaceText) {
		return resolvedName, false, nil
	}
	keyboardMap[iTerm2OptionBackspaceKey] = map[string]any{
		"Action": float64(11),
		"Text":   iTerm2OptionBackspaceText,
	}
	return resolvedName, true, nil
}

func selectIterm2Profile(data map[string]any, bookmarks []any, profileName string) (map[string]any, string, error) {
	if strings.TrimSpace(profileName) != "" {
		if profile := findIterm2ProfileByName(bookmarks, profileName); profile != nil {
			return profile, profileName, nil
		}
	}
	if guid, ok := data["Default Bookmark Guid"].(string); ok && guid != "" {
		if profile := findIterm2ProfileByGuid(bookmarks, guid); profile != nil {
			return profile, iterm2ProfileName(profile), nil
		}
	}
	for _, bookmark := range bookmarks {
		if profile, ok := bookmark.(map[string]any); ok {
			return profile, iterm2ProfileName(profile), nil
		}
	}
	return nil, "", errors.New("iTerm2 preferences do not contain a writable profile")
}

func findIterm2ProfileByName(bookmarks []any, name string) map[string]any {
	for _, bookmark := range bookmarks {
		profile, ok := bookmark.(map[string]any)
...
func findIterm2ProfileByGuid(bookmarks []any, guid string) map[string]any {
	for _, bookmark := range bookmarks {
		profile, ok := bookmark.(map[string]any)
		if ok && profile["Guid"] == guid {
			return profile
		}
	}
	return nil
...
func iterm2ProfileName(profile map[string]any) string {
	if name, ok := profile["Name"].(string); ok && name != "" {
		return name
	}
	if guid, ok := profile["Guid"].(string); ok && guid != "" {
		return guid
	}
	return "selected profile"
}

func iterm2KeyboardEntryMatches(entry map[string]any, text string) bool {
	return fmt.Sprint(entry["Action"]) == "11" && entry["Text"] == text
}
