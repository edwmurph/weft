...
Task rows may use subtle row color and marker shape to make derived state easier to scan. Rows for active non-ready/non-idle task states (`starting`, `running`, `waiting`, `working`, `shipping`, or newer live task states) replace the static marker with the shared high-resolution Braille loading spinner frame. Rows whose PTY has not produced visible content may also use the spinner until the task is ready. Configured command task rows also use the same spinner while a submitted foreground command is in progress, returning to the ready marker when the shell regains the PTY foreground process group. Active/loading rows may render a concise elapsed operation duration such as `12s`, `2m`, or `1h2m` immediately after the task type badge from supervisor snapshot metadata; this metadata is runtime-only and must be omitted for ready, idle, stopped, killed, and error rows. Codex screens detected as ready user prompts, including tool permission prompts and command approval prompts, use the ready marker instead of the loading spinner. Ready or idle rows use the subtle `·` marker and keep their ready color even when selected, hovered, or also the active task; the selected ready-row variant must use enough foreground/background contrast to stay readable. Stopped rows use `◦`; errors use `!` and the error marker/color. These visuals are presentation only and must not add status text.
