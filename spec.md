...
Homebrew publishing keeps separate owner and contributor paths. Normal maintainer releases from Weft `main` start only after the push-triggered `CI` workflow succeeds on `main`; manually dispatched CI runs must not trigger publishing. The Weft publish workflow checks out the exact commit that passed push-based CI, writes the formula update to `edwmurph/homebrew-tap` `main`, dispatches the tap direct bottle workflow, waits for an Apple Silicon macOS bottle build, uploads the bottle to the tap GitHub Release, and verifies the formula `bottle do` block. Contributor or manual tap changes still use the standard pull-request bottle flow: the tap `brew test-bot` workflow builds bottles for supported PR runners, and the tap `brew pr-pull` workflow uploads bottles to GitHub Releases, merges the formula update, and writes the formula `bottle do` block. The formula must still keep its source `url`, `sha256`, and Go build instructions so `brew install --build-from-source edwmurph/tap/weft` remains a fallback.

The `CI` workflow is the repository quality gate. It can be manually dispatched for safe validation without publishing. Pull requests and pushes that change source, workflow, script, test, or specification files run the full Ubuntu and macOS matrix, including live integration tests and build. Documentation or process-only changes may use the minimal CI path, but the single `CI Gate` job remains the required status check and fails if any required validation job fails.
...
When at least one workspace exists and there is enough vertical room, render a template card under the last workspace card with an italic plus-sign title and concise italic copy telling the user to press the configured add-workspace key to create a workspace. Mouse hover or click on the template card does not change dashboard focus or selection. The template card is selectable by moving down from the last workspace card: real workspace cards return to their inactive border, and the Tasks pane renders the same empty state as no selected workspace while Task Live Preview renders `No task selected`. Pressing `Enter` while the template card is selected opens the same add-workspace prompt as the configured add-workspace key. If that prompt is canceled, focus returns to the selected template card instead of jumping to a real workspace.
...
When no workspace exists or no workspace is selected, the Tasks pane shows centered help text telling the user to add a workspace first. It must not advertise creating a task until a workspace exists. When a workspace is selected, the top of the Tasks pane renders a selectable italic template row with a plus-sign label and concise copy telling the user to press the configured new-task key to create a task. Mouse hover or click on the template row does not change dashboard focus or selection. Pressing `Enter` while the template row is selected opens the same new-task form as the configured new-task key.
...
- verify focused Task Console mouse wheel scrollback, ignored Task Live Preview wheel input, Task Live Preview drag-copy selection, and focused-console drag-copy selection
