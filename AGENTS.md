...
- When product or docs changes alter workflows shown in the README demo, explicitly call out whether the demo video should be regenerated; use `$weft-demo-video` in `.agents/skills/weft-demo-video/` for recording or regeneration work.
...
- For broad refactor requests, follow the normal Weft worktree, bounded-scope, docs/spec-sync, and verification rules in this file. Do not rely on a repo-local refactor skill or durable refactor suggestion log.
- After verified implementation work, summarize what changed, what verification passed, exactly how the user can test it locally, include the exact command(s) to retest the changed behavior, and offer to ship it. Before offering to ship, explicitly state whether the change would be breaking for installed users; if yes, name the user-visible impact and the migration/recovery action that will appear in release notes. Then stop for review unless the user explicitly says **"ship it"**.
- When the user says **"ship it"**, interpret it as: squash-merge the already-reviewed change to `main`, push `main` to `origin/main`, watch the `CI` workflow for the pushed commit, then watch the `Publish Homebrew` workflow that starts after successful push-based CI, and report the commit, release, tap publish status, verification status, and a brief re-explanation of the shipped user-visible change in the same message.

## Ship It Flow

- Before starting the ship flow, inspect GitHub Actions for `CI` and `Publish Homebrew` runs on `main`. If either workflow has a queued or in-progress run, wait for it to finish before doing any local landing, commit, or push work. If the latest completed run for either workflow failed, stop and report that failure to the user instead of shipping a new change on top of a broken validation or release path.
...
- After pushing, watch the latest `CI` run for the pushed `main` commit with `gh run watch --exit-status`, then watch the `Publish Homebrew` run that starts from that successful push-based CI run. Manually dispatching `CI` is safe and must not publish. Treat ship as complete only after CI succeeds and the publish workflow succeeds, creates the GitHub release, publishes the tap bottle through the direct owner workflow or the tap PR workflow, updates the Homebrew tap formula, and writes a matching `bottle do` block.
